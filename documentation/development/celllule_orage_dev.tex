 ...
 L'alimentation en énergie du système est importante. Nous avons choisi de travailler avec une batterie LiFePO4 au format AA (ou une pile AA, l'idée étant de pouvoir remplacer facilement l'alimentation sur le terrain). Pour pouvoir travailler efficacement avec les ampli-ops il faut une tension plus élevée que celle de batterie (soit 3.3 [V] (ou 1.5 [V] pour une pile AA). J'ai ajouté au schéma un convertisseur DC-DC qui permet de travailler le plus efficacement possible avec une source dont la tension varie au fil de sa décharge. Il comporte également le circuit de charge de la batterie, les informations complètes à ce sujet sont données dans le chapitre correspondant.\\
 En alimentant le circuit avec la batterie choisie, dont la capacité est de 1.1 [Ah], on obtient une autonomie de XX [h] en activité (donc prêt à déclencher au moindre éclair) ou de XX [h] en déclenchement (c'est à dire retour utilisateur - LED et buzzer - actifs et déclencheur actif).\\
 Pour la régulation de l'alimentation du circuit, j'ai choisi le MAX857ESA+ qui permet de travailler dès 0.8 [V] et qui assure un courant de sortie de 500 [mA]. \\
...
Les batteries de technologie LiFePO4 sont considérées comme déchargées au-dessous de 2.8 [V] \footnote{Selon la page Wikipédia correspondante ainsi que divers sites spécialisés}, ainsi nous sommes en permanence dans la plage de fonctionnement du convertisseur.\\
